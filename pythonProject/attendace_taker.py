...
#     #     conn.close()
#
# #attendance method issue with the sending email to the present student
#     def attendance(self, name):
#         current_date = datetime.datetime.now().strftime('%Y-%m-%d')
#         conn = sqlite3.connect("attendance.db")
#         cursor = conn.cursor()
...
#             print(f"{name} marked as present for {current_date} at {current_time}")
#
#             # Check if the student is known (not 'unknown') and add to absent_students if absent
#             if name != 'unknown':
...
#         self.process(cap)
#
#         # Send email notification after quitting the application
...
#
#     @staticmethod
#     def return_cosine_similarity(feature_1, feature_2):
#         similarity = 1 - cosine(feature_1, feature_2)
#         return similarity
#
#     def centroid_tracker(self):
#         for i in range(len(self.current_frame_face_centroid_list)):
...
#
#             for j in range(len(self.last_frame_face_centroid_list)):
#                 similarity = self.return_cosine_similarity(
...
#                             for i in range(len(self.face_features_known_list)):
#                                 if str(self.face_features_known_list[i][0]) != '0.0':
#                                     similarity_tmp = self.return_cosine_similarity(
#                                         self.current_frame_face_feature_list[k],
#                                         self.face_features_known_list[i]
#                                     )
