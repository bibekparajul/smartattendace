...
class Face_Register:
    def __init__(self):
        ...
        self.font_step_title = tkFont.Font(family='Courier New', size=15, weight='bold')
        self.font_warning = tkFont.Font(family='Arial', size=15, weight='bold')
    ...
    #     tk.Label(self.frame_right_info,
    #              font=self.font_step_title,
    #              text="Input Details").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
    #
    ...
    #     os.makedirs(self.current_face_dir)
    #     # Store the email
    #     with open(self.current_face_dir + "/info.txt", "w") as info_file:
    #         info_file.write("Name: {}\n".format(self.input_name_char))
    #         info_file.write("Email: {}\n".format(email))
    #     self.log_all["text"] = "\"" + self.current_face_dir + "/\" created!"
    #     logging.info("\n%-40s %s", "Create folders:", self.current_face_dir)
    #     self.ss_cnt = 0
    ...
    # Modify the create_face_folder method to include email in folder name
    def create_face_folder(self, email):
        self.existing_faces_cnt += 1
        if self.input_name_char:
            person_name = "person_" + str(self.existing_faces_cnt) + "_" + self.input_name_char
        else:
            person_name = "person_" + str(self.existing_faces_cnt)

        # Append email to the folder name
        person_name_with_email = f"{person_name}_{email}"

        self.current_face_dir = os.path.join(self.path_photos_from_camera, person_name_with_email)
        os.makedirs(self.current_face_dir)
